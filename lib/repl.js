...
export default class REPL {
    ...
    tidalSendLine(command) {
        ...
        if (command.includes("chaos")) {
            this.sendChaosLine(command);
        } else {
    ...
    tidalSendExpression(expression) {
        ...
        //alert("expression does not contain chaosVector");
        this.tidalSendLine(':{');
    ...
    sendChaosCmd(command){
        this.stdinWrite(command);
        this.stdinWrite('\n');
    }

    sendChaosLine(command) {
        // Get the channel we entered on the command line
        var channel = command.substring(1,2);
        alert("channel=" + channel);
        // Try to open the file associated with this channel
        var notes = fs.readFileSync('/home/dshakes/projects/tidal-chaos/src/Sound/Tidal/Chaos/Bank/birfXVals-' + channel + '.txt').toString();
        // Strip out any spurious carriage return or line feeds from string
        notes = notes.replace(/(\r\n|\n|\r)/gm, "");
        // Put brackets around
        notes = "[" + notes + "]";
        notes = JSON.stringify(notes);
        command = command.replace("chaos", "chaos " + notes);
        alert("sending chaosline=" + command);
        this.sendChaosCmd(command);
    }
}
